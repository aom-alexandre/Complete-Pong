...
void setup(){
  ...
  print("P1 uses 'W' to go up and 'S' to go down, P2 uses the arrows, 'R' restarts the game and you can increase/decrease speed with 'T'/'Y', increase.");
  
  drawBase();
  
  boti = loadImage("bot.jpg");  
  image(boti, width/4 - boti.width/2, height/2 - boti.height/2); 
  playi = loadImage("play.jpg");
  image(playi, width/4 * 3 - playi.width/2, height/2 - playi.height/2);
}

boolean startbot(){
  if ((mouseX <= width/4 + boti.width/2) && (mouseX >= width/4 - boti.width/2) && (mouseY <= height/2 + boti.height/2) && (mouseY >= height/2 - boti.height/2) && (mousePressed))
    return true;
  else
    return false;
}
...
boolean startplay(){
  if ((mouseX <= width/4 * 3 + playi.width/2) && (mouseX >= width/4 * 3 - playi.width/2) && (mouseY <= height/2 + playi.height/2) && (mouseY >= height/2 - playi.height/2) && (mousePressed))
    return true;
  else
    return false;
}
...
void drawCPU(){
  ...
  if (y + ball.height <= p1 + left.height){
  p1 = p1 -5;
  }
}
...
void pointsB(){
  ...
  p1 = height/2 - left.height/2;
  p2 = height/2 - right.height/2;
  image(lost, width/2 - lost.width/2, height/2 - lost.height/2);
  noLoop();
  }
  ...
  p1 = height/2 - left.height/2;
  p2 = height/2 - right.height/2;
  image(won, width/2 - won.width/2, height/2 - won.height/2);
  noLoop();
  }
